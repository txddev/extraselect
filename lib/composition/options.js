...
export const loadOptions = (originalNode,url = null) => {
    ...
    if(url != null){
        fetch(url).then(async res => {
            for(let opt of await res.json()){
                
                options.actions.push(opt.key,opt.value+opt.key.toString(),opt.data)
            }
            options.actions.sort((a,b) => a.value.localeCompare(b.value))
        })
    }
