...
const exposeOptions = (optionRef,selectedRef,name) => {
...
export const loadOptions = (originalNode,propOptions,propValue,initialValue, optionKey = null ) => {
        ...
        if(originalNode.matches("select")){
            ...
            options.value = Array.apply(null, originalNode.options).reduce((c,opt) => {
                c.push({
                    ...
                    data: Object.keys(opt.dataset).reduce((c,key) => {
                        c[key] = stringOrJson(opt.dataset[key])
                        return c
                    },{}) 
                })
                return c
