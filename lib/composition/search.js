...
const fetchOptions = async function (url,searchTerm = null,options = {}){
    const opts = {method: 'POST',credentials: "same-origin",...options}
    opts.body = JSON.stringify({search:searchTerm,...opts.body})  
    const response = await fetch(url,opts)
    return await response.json()
}

export const loadSearch = (options, url, searchableUrl,filterText, minChars,fetchMode = "limited",fetchData = {}) => {
