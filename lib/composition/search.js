    ...
    if(url != null && url.length > 0){
        ...
        if(searchableUrl){
            ...
            watchEffect((onCleanup)=>{
                const hash = JSON.stringify(filterValues.value) ?? "default"
                
                if(searchedTerms[hash] == null){
                    searchedTerms[hash] = []
                }
                if(filterText.value.length>= minChars){
                    ...
                    switch(fetchMode){
                        ...
                        default:
                        case 'limited':
                            shouldFetch = !searchedTerms[hash].includes(filterText.value)
                            break;
                        case 'complete':
                            shouldFetch = searchedTerms[hash].reduce((c,term)=>c&&!filterText.value.startsWith(term),true)
                            break;
                    }
                    if(shouldFetch){
                        searchingBool.value = true
                        const debounce = setTimeout(()=>{
                            searchedTerms[hash].push(filterText.value)
