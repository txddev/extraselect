...
export const loadFilter = (options,selectedOptions,toggleOption, filterFields = [], hardFilterFields = [], matchType = "exact") => {
            ...
            node.addEventListener("change", (event) => { 
                ...
                if(isHard){
                    nextTick(() =>{
                        ...
                        } else if(!filteredOptions.value.find(el => el.key == filterText.value)){
                            toggleOption(filterText.value,false)
                        }
                    })
                }
            })
            ...
            results = results
                .filter((option)=>{
                    for(let key in filterValues.value){
                        ...
                        if(applyFilter && option.data?.hasOwnProperty(key)){
                            if(Array.isArray(option.data[key])){
                                if(!option.data[key].includes(filterValues.value[key])){
                                    return false
                                }
                            }else if(option.data[key] != filterValues.value[key]){
                                return false
                            }
                        }
                    }
            ...
            switch(matchType){
                case "loose":
                    ...
                    results = results.filter((option) => {
                        ...
                        return searchWords.every(word => optionValue.includes(word));
                    });
                    break;
                default:
                    results = results
